...
class DuplicateFinderDialog(QDialog):
    def __init__(self, parent=None, tracks=None):
        super(DuplicateFinderDialog, self).__init__(parent)
        self.tracks = tracks
        ...
        # Criteria selection
        layout.addWidget(QLabel(f"<b>{translate('Dupes', 'Select fields to compare:')}</b>"))
    ...
    def find_duplicates(self):
        fields = []
        if self.artist_cb.isChecked(): fields.append('artist')
        if self.title_cb.isChecked(): fields.append('title')
        if self.album_cb.isChecked(): fields.append('album')
        if self.length_cb.isChecked(): fields.append('__length')
    ...
    def display_results(self):
        ...
        for group in self.duplicate_groups:
            for t in group:
                ...
                self.results_table.setItem(row, 0, QTableWidgetItem(str(t.get('artist', [''])[0])))
                self.results_table.setItem(row, 1, QTableWidgetItem(str(t.get('title', [''])[0])))
                self.results_table.setItem(row, 2, QTableWidgetItem(str(t.get('album', [''])[0])))
    ...
    def select_in_main(self):
        ...
        for index in selected_rows:
            track = self.results_table.item(index.row(), 0).data(Qt.ItemDataRole.UserRole)
