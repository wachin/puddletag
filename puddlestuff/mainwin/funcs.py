...
from ..util import split_by_tag, to_string, translate
...
def check_copy_data(data):
    ...
    if len(data) > 5 * 1024 * 1024:
        ...
        yesBtn = msgbox.addButton(
            translate("Defaults", "&Yes"), QMessageBox.ButtonRole.YesRole
        )
        noBtn = msgbox.addButton(
            translate("Defaults", "No"), QMessageBox.ButtonRole.NoRole
        )
        msgbox.setDefaultButton(noBtn)
        msgbox.setEscapeButton(noBtn)
        noImgBtn = msgbox.addButton(
            translate("Messages", "Copy without images."),
    ...
    if state:
        if not status["library"]:
            QMessageBox.critical(
                parent,
                translate("MusicLib", "No libraries found"),
        ...
        for artist, tracks in split_by_tag(files, "artist", None).items():
            if artist in libartists:
                ...
                titles = [
                    track["title"][0].lower() if "title" in track else ""
                    for track in libtracks
                ]
                for track in tracks:
...
def number_tracks(
    ...
    restartdirs: bool,
    padlength: int,
    split_field: str = "__dirpath",
    output_field: str = "track",
    by_group: bool = False,
) -> None:
    ...
    if not split_field:
        QMessageBox.critical(
            parent,
            translate("Autonumbering Wizard", "Field empty..."),
            translate(
    ...
    if not output_field:
        QMessageBox.critical(
            ...
            translate(
                "Autonumbering Wizard",
                "The output field specified was invalid. Please check your values.",
            ),
        )
        return
...
def paste_onto():
    ...
    emit(
        "writeselected",
        (
            dict(list(zip(s, list(cliptag.values()))))
...
def run_function(parent=None, prevfunc=None):
    ...
    if prevfunc:
        ...
        f = actiondlg.CreateFunction(
            parent=parent, selected_fields=selected_fields, example=example, text=text
        )
...
def search_replace(parent=None):
    ...
    dialog = actiondlg.CreateFunction(
        prevfunc=func,
        parent=parent,
        selected_fields=list(selected.keys()),
        example=audio,
        text=text,
    )
...
def update_status(enable=True):
    ...
    try:
        ...
        if newfilename:
            ...
            emit(
                "tfstatus",
                translate("Status Bar", "New Filename: <b>{}</b>").format(
    ...
    try:
        ...
        if newfolder:
            ...
            dirstatus = translate(
                "Dir Renaming", "Rename: <b>{}</b> to: <i>{}</i>"
...
obj.receives = [("filesselected", update_status), ("patternchanged", update_status)]
