...
class StatisticsDialog(QDialog):
    def __init__(self, parent=None, tracks=None):
        ...
        total_length = sum(
            audioinfo.util.lnglength(t.get("__length", "00:00")) for t in tracks
        )

        info_layout.addWidget(
            QLabel(f"<b>{translate('Statistics', 'Total Tracks:')}</b> {total_tracks}")
        )
        info_layout.addWidget(
            QLabel(
                f"<b>{translate('Statistics', 'Total Size:')}</b> {audioinfo.util.str_filesize(total_size)}"
            )
        )
        info_layout.addWidget(
            QLabel(
                f"<b>{translate('Statistics', 'Total Duration:')}</b> {audioinfo.util.strlength(total_length)}"
            )
        )
        ...
        # Genre Distribution
        layout.addWidget(
            QLabel(f"<h3>{translate('Statistics', 'Genre Distribution')}</h3>")
        )
        ...
        # Format Distribution
        layout.addWidget(
            QLabel(f"<h3>{translate('Statistics', 'Format Distribution')}</h3>")
        )
        ...
        layout.addWidget(
            QLabel(f"<h3>{translate('Statistics', 'Bitrate Distribution')}</h3>")
        )
    ...
    def populate_table(self, table, field):
        ...
        table.setHorizontalHeaderLabels(
            [translate("Statistics", "Value"), translate("Statistics", "Count")]
        )
