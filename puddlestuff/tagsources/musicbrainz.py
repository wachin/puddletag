...
def parse_label_list(release_node):
    labels = parse_node(release_node, "label-info-list", "label-info", "label")

    catalogs = [z["catalog-number"] for z in labels if "catalog-number" in z]
    label_names = [
        z["label"]["name"] for z in labels if "label" in z and "name" in z["label"]
    ]
    label_ids = [
        z["label"]["id"] for z in labels if "label" in z and "id" in z["label"]
    ]
    return {
        "label": label_names,
...
def parse_track_list(node):
    ...
    for i, t in enumerate(parse_node(node, "track-list", "track", "position")):
        ...
        for k, v in list(track.items()):
            if (
                not isinstance(track[k], (str, list))
...
def search_album(album=None, artist=None, limit=25, offset=0, own=False):
    ...
    if album:
        if isinstance(album, str):
            album = solr_escape(album)
        if artist:
            query = "release:" + urllib.parse.quote_plus(album) + "%20AND%20" + query
        else:
...
class MusicBrainz:
    ...
    def __init__(self):
        ...
        self.preferences = [
            ...
            [
                translate("MusicBrainz", "Cover size to retrieve:"),
                COMBO,
                [
                    [
    ...
    def keyword_search(self, s):
        if s.startswith(":a"):
            ...
            try:
                url = search_album(
                    "arid:" + solr_escape(artist_id), limit=100, own=True
                )
                return parse_album_search(urlopen(url))
            except RetrievalError as e:
                msg = translate(
                    "MusicBrainz", "<b>Error:</b> While retrieving {}: {}"
            ...
            try:
                ...
                msg = translate(
                    "MusicBrainz", "<b>Error:</b> While retrieving Album ID {} ({})"
        ...
        if hasattr(artists, "items"):
            ...
            if album_id:
                try:
                    write_log(
                        translate(
                            "MusicBrainz", "Found album id {} in tracks. Retrieving"
                        ).format(album_id)
                    )
    ...
    def retrieve_covers(self, album_id):
        ...
        if self.__num_images == 0:
            try:
                ...
                print()
                write_log(
                    translate("MusicBrainz", "Error retrieving image: {}").format(
                        str(e)
                    )
                )
                return []
