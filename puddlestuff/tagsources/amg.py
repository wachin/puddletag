...
def create_search(terms):
    ...
    return search_adress % urllib.parse.quote(re.sub(r"(\s+)", " ", terms))
...
def parse_sidebar(sidebar):
    ...
    if moods is not None:
        info["mood"] = [
            convert(z.string) for z in moods.find_all("span", {"class": "mood"})
        ]

    themes = sidebar.find("section", {"class": "themes"})
    if themes is not None:
        info["theme"] = [
            convert(z.string) for z in themes.find_all("span", {"class": "theme"})
        ]
...
def parse_search_element(td, id_field=ALBUM_ID):
    ...
    info["#extrainfo"] = [info["album"] + " at AllMusic.com", info["#albumurl"]]
...
def parse_searchpage(page, artist=None, album=None, id_field=ALBUM_ID):
    ...
    if artist and album:
        ...
        if not top:
            top = [album for album in albums if equal(d, album, False, ["album"])]
    elif artist:
        d = {"artist": artist}
        top = [album for album in albums if equal(d, album, True, ["artist"])]
        if not top:
            top = [album for album in albums if equal(d, album, False, ["artist"])]
    else:
        top = []
...
def parse_track_table(table, discnum=None):
    ...
    headers = [
        th.element.attrib.get("class", convert(th.string)) for th in header_items
    ]
