...
def joinuntil(cursor, text):
    ...
    al = cursor.all_lines
    cursor.all_lines = (
        al[: cursor.lineno] + ["".join(ret)] + append + al[cursor.lineno + len(ret) :]
    )
    cursor.lineno = cursor.lineno
...
def _set(cursor, field, value=None):
    if not value:
        ...
        cursor.output[field] = value if isinstance(value, str) else str(value)
...
def _while(cursor, condition, numtimes=None):
    ...
    if cursor.line[cursor.charno :].startswith(condition):
        for i, (cmd, lineno, args) in enumerate(
            reversed(cursor.source[: cursor.cmd_index])
        ):
            if cmd == "do":
                if nested > 0:
                    nested -= 1
                    continue
                if numtimes is None or numtimes >= cursor.num_iters:
    ...
    if cursor.tracks == {}:
        cursor.tracks = [{"title": z.strip()} for z in cursor.cache.split("|")]
