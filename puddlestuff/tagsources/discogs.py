...
IMAGEKEYS = {"SmallImage": SMALLIMAGE, "LargeImage": LARGEIMAGE}
...
TRACK_KEYS = {"position": "track", "duration": "__length", "notes": "discogs_notes"}
...
__minimum_request_separation = (
    1  # second - no more than one request sent to discogs in this time frame
)
__reference_time = (
    0  # A reference time. The initial value ensure first request goes ahead
)
...
def parse_album_json(data):
    ...
    info["catno"] = [
        _f for _f in (z.get("catno") for z in data.get("labels", [])) if _f
    ]
...
def retrieve_album(info, image=LARGEIMAGE, rls_type=None):
    ...
    if isinstance(info, int):
        r_id = str(info)
        info = {}
        write_log(translate("Discogs", "Retrieving using Release ID: {}").format(r_id))
        rls_type = "release"
        ...
        r_id = info["#r_id"]
        write_log(translate("Discogs", "Retrieving album {}").format(info["album"]))
    ...
    if image in IMAGE_TYPES and "#cover-url" in info:
        ...
        for large, small in info["#cover-url"]:
            if image == LARGEIMAGE and large:
                write_log(translate("Discogs", "Retrieving cover: {}").format(large))
                try:
                    data.append({DATA: urlopen(large)})
                except RetrievalError as e:
                    write_log(translate("Discogs", "Error retrieving image:") + str(e))
            else:
                write_log(translate("Discogs", "Retrieving cover: {}").format(small))
                try:
    ...
    try:
        info["#extrainfo"] = (
            translate("Discogs", "{} at Discogs.com").format(info["album"]),
            site_url,
        )
    except KeyError:
        pass
...
class Discogs:
    ...
    def __init__(self):
        ...
        self.preferences = [
            ...
            [
                translate("Discogs", "Cover size to retrieve"),
                COMBO,
                [[translate("Discogs", "Small"), translate("Discogs", "Large")], 1],
            ],
    ...
    def search(self, album, artists):
        if hasattr(artists, "values"):
            ...
            if not album_id:
                ...
                write_log(translate("Discogs", "Found Discogs ID: {}").format(album_id))
