...
def to_utf8(parent=None):
    if status["previewmode"]:
        QMessageBox.information(
            ...
            QApplication.translate(
                "Previews", "You need to disable preview mode first."
            ),
        )
        return
    ...
    def func():
        for row, f in zip(rows, files):
            try:
                ...
                m = str(
                    QApplication.translate(
                        "Defaults", "An error occured while converting <b>{}</b>. ({})"
    ...
    s = progress(func, QApplication.translate("ID3 Plugin", "Converting "), len(files))
    s(parent)
...
def update_to_24(parent=None):
    ...
    def func():
        for row, f in zip(rows, files):
            try:
                ...
                m = str(
                    QApplication.translate(
                        "Defaults", "An error occured while updating <b>{}</b>. ({})"
                    ).format(filename, e.strerror)
                )
                if row == rows[-1]:
                    yield m, 1
    ...
    s = progress(func, QApplication.translate("ID3 Plugin", "Updating "), len(files))
    s(parent)
