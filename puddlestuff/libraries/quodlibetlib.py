...
class QuodLibet:
    def __init__(self, filepath):
        ...
        for track in self._tracks:
            cached[track.get("artist", "")][track.get("album", "")].append(track)
        self._cached = cached
    ...
    def get_tracks(self, maintag, mainval, secondary=None, secvalue=None):
        ...
        if secondary and secvalue:
            if secondary == "album" and maintag == "artist":
                tracks = (
                    Tag(self, track)
                    for track in self._cached[mainval][secvalue]
                    if exists(track)
                )
            ...
            if maintag == "artist":
                ...
                return [Tag(self, track) for track in tracks if exists(track)]
...
class InitWidget(QWidget):
    ...
    def library(self):
        ...
        try:
            ...
            raise MusicLibError(
                0, translate("QuodLibet", "{} ({})").format(e.strerror, e.filename)
            )
        except (pickle.UnpicklingError, EOFError):
            raise MusicLibError(
                0,
                translate(
                    "QuodLibet", "{} is an invalid QuodLibet music library file."
                ).format(dbpath),
            )
