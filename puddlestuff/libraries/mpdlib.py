    ...
    def load(self):
        ...
        try:
            ...
            for song in songs:
                ...
                tag = {
                    'artist': song.get('artist', [''])[0] if isinstance(song.get('artist'), list) else song.get('artist', ''),
                    'album': song.get('album', [''])[0] if isinstance(song.get('album'), list) else song.get('album', ''),
                    'title': song.get('title', [''])[0] if isinstance(song.get('title'), list) else song.get('title', ''),
                    'track': song.get('track', [''])[0] if isinstance(song.get('track'), list) else song.get('track', ''),
                    'genre': song.get('genre', [''])[0] if isinstance(song.get('genre'), list) else song.get('genre', ''),
                    'path': song['file']
                }
