...
from PyQt6.QtWidgets import (QDialog, QVBoxLayout, QHBoxLayout, QLabel, 
                             QComboBox, QTextEdit, QPushButton, QFileDialog, QMessageBox)
from .translations import translate
from .puddleobjects import PuddleConfig
from .util import encode_fn, decode_fn
...
$loopend()</table>
    ...
    def save_template(self):
        ...
        name, ok = QInputDialog.getText(self, translate("Export", "Save Template"),
    ...
    def perform_export(self):
        if not self.tracks:
            QMessageBox.warning(self, "puddletag", translate("Export", "No tracks selected for export."))
            return
        ...
        filename, _ = QFileDialog.getSaveFileName(self, translate("Export", "Save Export As..."), "", file_filter)
        if not filename:
            return
        ...
        try:
            ...
            with open(filename, 'w', encoding='utf-8') as f:
                f.write(result)
            QMessageBox.information(self, "puddletag", translate("Export", "Export completed successfully."))
            self.accept()
        except Exception as e:
            traceback.print_exc()
            QMessageBox.critical(self, "puddletag", translate("Export", "Error during export: {}").format(str(e)))
    ...
    def process_template(self, template, tracks):
        ...
        if loop_match:
            ...
            return template[:loop_match.start()] + loop_result + template[loop_match.end():]
        else:
